...
def test_load_dotenv_if_present_sets_vars(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("FIXER_DOTENV_TEST_KEY", raising=False)
    (tmp_path / ".env").write_text("FIXER_DOTENV_TEST_KEY=fromfile\n", encoding="utf-8")
    run_module.load_dotenv_if_present(str(tmp_path))
    assert os.environ.get("FIXER_DOTENV_TEST_KEY") == "fromfile"


def test_load_dotenv_if_present_skips_missing(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("FIXER_DOTENV_TEST_KEY", raising=False)
    run_module.load_dotenv_if_present(str(tmp_path))
    assert os.environ.get("FIXER_DOTENV_TEST_KEY") is None


def test_load_dotenv_if_present_respects_setdefault(
    tmp_path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setenv("FIXER_DOTENV_TEST_KEY", "preset")
    (tmp_path / ".env").write_text("FIXER_DOTENV_TEST_KEY=fromfile\n", encoding="utf-8")
    run_module.load_dotenv_if_present(str(tmp_path))
    assert os.environ.get("FIXER_DOTENV_TEST_KEY") == "preset"
